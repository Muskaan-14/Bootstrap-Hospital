...
<body>
        ...
        <ul class="nav">
          ...
          <li class="nav-item">
            <a class="nav-link" href="appointment.php" style="border-top: 2px solid rgb(255,142,75);">
              ...
              <span class="menu-title text-dark">Appointment</span>
            </a>
          </li>
          <li class="nav-item">
          ...
          <li class="nav-item">
            <a class="nav-link" href="doctors.php" style="background: rgb(255,142,75);">
              <i class="icon-help menu-icon text-white"></i>
              <span class="menu-title text-white" style="color: rgb(22,30,84);">Doctors</span>
            </a>
          </li>
          <li class="nav-item">
                            ...
                            <td>
                              ...
                              if ($row['status'] == "Active") {
                                ...
                                <button type="submit" class="w-100 btn text-white" style="background: rgb(60,196,123)"
                                  onclick="togglestatus(this.value,<?php echo $row['doctor_id'] ?>)"
                                  value="<?php echo $row['status'] ?>" id="doctor_toggle<?php echo $row['doctor_id'] ?>">
                                  <?php echo $row['status'] ?>
                                </button>
                                <?php
                              } else {
                          ...
                          <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h1 class="modal-title fs-5" id="doctorModalLabel">Add Service</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                  aria-label="Close"></button>
                              </div>
                              <form action="doctor_table.php" method="post" enctype="multipart/form-data">
                                <div class="modal-body">
                                  Image :
                                  <input class="file-upload-input mb-2 w-100" name="image" id="image" type="file"
                                    Required>
                                  Name :
                                  <input class="p-2 border rounded-3 w-100 mb-2" name="name" type="text"
                                    placeholder="Name" Required>
                                  Degree :
                                  <input class="p-2 border rounded-3 w-100 mb-2" name="degree" type="text"
                                    placeholder="Degree" Required>
                                  Speciality :
                                  <input class="p-2 border rounded-3 w-100 mb-2" name="speciality" type="text"
                                    placeholder="Speciality" Required>
                                  Patients :
                                  <input class="p-2 border rounded-3 w-100 mb-2" name="patients" type="number"
                                    placeholder="Patients" Required>
                                </div>
  ...
  <script>
    function togglestatus(data, id) {
      ...
      req.open("GET", "http://localhost/Bootstrap%20Hospital/Backend/pages/tables/doctor_table_toggle.php?last=" + data + "&id=" + id, true);
      req.send();
      req.onreadystatechange = function () {
        if (req.readyState == 4 && req.status == 200) {
          document.getElementById("doctor_toggle" + id).value = req.responseText;
          document.getElementById("doctor_toggle" + id).innerHTML = req.responseText;
          req.open("GET", "http://localhost/Bootstrap%20Hospital/Backend/pages/tables/service_table_toggle_color.php?last=" + data, true);
          req.send();
          req.onreadystatechange = function () {
            if (req.readyState == 4 && req.status == 200) {
              document.getElementById("doctor_toggle" + id).style.backgroundColor = req.responseText;
            }
          }
        }
      }
